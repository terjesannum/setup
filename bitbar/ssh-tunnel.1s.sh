...
for h in $(hosts ~/.ssh/config); do
    if pgrep -qf "ssh -fNq ${h}"; then
        echo "${h} | color=green bash=/usr/bin/pkill param1='-f' param2='\"ssh -fNq ${h}\"' terminal=false"
    else
        echo "${h} | color=red bash=/usr/bin/ssh param1='-fNq' param3=${h} terminal=false"
    fi
done
